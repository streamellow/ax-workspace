...
from gmail_classifier import scrape_job_page
...
if "scraped_results" not in st.session_state:
    ...
    for i, posting in enumerate(postings):
        ...
        sections = scrape_job_page(url) if url else []
...
st.markdown(
    "<hr style='margin:4px 0 12px 0;border-color:#ddd'>",
...
for i, r in enumerate(results):
    ...
    # expander 제목 = 클릭 가능한 링크 텍스트
    expander_label = f"{i+1}.  {title}  |  {company}  |  {location}  {status_label}"
